...
class SVD_Newton(nn.Module):
     """Forward Pass: SVD
        Backward Pass: BP of Newton-Schulz iteartion
     ...
     def __init__(self, iterNum=3, is_sqrt=True, is_vec=True, input_dim=2048, dimension_reduction=None):
         ...
         super(SVD_Newton, self).__init__()
...
class Sqrtm(Function):
     ...
     def forward(ctx, input, iterN):
         # Both SVD and NS iteration are calculated in the FP but only use SVD
         # the intermediate results of NS iteration are used for BP
         x = input
         batchSize = x.data.shape[0]
         dim = x.data.shape[1]
         dtype = x.dtype
         ...
         _,eig_diag,eig_vec=torch.svd(x, some=True, compute_uv=True)
...
class Triuvec(Function):
     ...
     def forward(ctx, input):
         ...
         y = x[:,index].float() #Change back to float precision
