...
def main():
    ...
    if args.benchmark.startswith('ILSVRC2012') or args.benchmark.startswith('imagenet'):
        ...
        train_dataset = Cub2011('/datasets-fgvc-cub/', train=True, transform=train_transforms,download=False)
        val_dataset = Cub2011('/datasets-fgvc-cub/', train=False, transform=val_transforms,download=False)
    elif args.benchmark.startswith('fgvc-aircraft'):
        train_dataset = Aircraft('/datasets-fgvc-aircraft/', train=True, transform=train_transforms, download=False)
        val_dataset = Aircraft('/datasets-fgvc-aircraft/', train=False, transform=val_transforms, download=False)
    elif args.benchmark.startswith('datasets-fgvc-cars'):
        train_dataset = Cars('/datasets-fgvc-cars/', train=True, transform=train_transforms, download=False)
        val_dataset = Cars('/datasets-fgvc-cars/', train=False, transform=val_transforms, download=False)
